    ...
    <div class="w-lg-500px p-10">
      <form class="form w-100" action="{{ route('register.village') }}" method="POST" id="loginForm">
        ...
        <div class="mb-11">
          ...
          <div class="text-gray-500 fw-semibold fs-6">Daftar sebagai admin kelurahan</div>
        </div>
        <div class="fv-row mb-8">
          <select id="kecamatan" class="form-select @error('district') is-invalid @enderror" name="district" data-control="select2" data-placeholder="Pilih Kecamatan">
            <option></option>
            @foreach ($district as $item)
            <option value="{{ $item->id }}" data-id="{{ $item->code }}" @if (old('district') == $item->id) selected @endif>{{ $item->name }}</option>
            @endforeach
          </select>
          @error('district')
...
<script>
  $(document).ready(function() {
    ...
    $('#kecamatan').on('change', function() {
      var district_code = $(this).find(':selected').data('id');
      $.ajax({
